...
public class MyPanel  extends JPanel implements KeyListener,Runnable  {
    ...
    public MyPanel(String key) {
        //セーブファイルの存在を判断します
        //存在の場合はプログラムを執行します、存在していなければkey＝"1"になります。
        File file = new File(Recorder.getRecordFile());
        if (file.exists()) {
            nodes = Recorder.getNodesEnemyTankRec();
        }else {
            System.out.println("セーブデーターが有りません、新しいゲームを開始します。");
            key="1";
        }
...
//音楽を再生します
        //new AePlayWave("/Users/garfield/Documents/JavaE-Leanning/Chapter20/src/1111.mp3").start();
    }
    //メソッドを新規し、撃墜数を表示ます。
